...
```{r}
# Example of summarizing reorder rate by product_id
library(dplyr)
...
sampled_data <- customer_data %>%
  sample_n(size = sample_size, replace = FALSE)  # Sample without replacement
ggplot(data = customer_data, aes(x = add_to_cart_order, y = reordered)) +
...
```{r}
# Calculate reorder frequency by order_id and product_id
reorder_freq <- customer_data %>%
  group_by(order_id, product_id) %>%
  summarise(reorder_count = sum(reordered)) %>%
  spread(key = product_id, value = reorder_count, fill = 0)

# Create heatmap
ggplot(data = reorder_freq, aes(x = order_id, y = factor(product_id), fill = reorder_count)) +
  geom_tile() +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(x = "Order ID", y = "Product ID",
       title = "Heatmap of Reorder Frequency by Order ID and Product ID")
...
```{r}
# Compute residuals
model_aug <- augment(model_fit, new_data = train_data, type.predict = "response", type.residuals = "deviance") %>%
...
ggplot(data = model_aug, aes(x = id, y = .resid)) + 
  geom_point() + 
  geom_hline(yintercept = 0, color = "red") + 
  labs(x = "id", 
       y = "Deviance residuals", 
